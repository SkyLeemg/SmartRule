...
import com.vitec.task.smartrule.view.large_img.BlockImageLoader.OnImageLoadListener;
import com.vitec.task.smartrule.view.large_img.factory.BitmapDecoderFactory;
import com.vitec.task.smartrule.view.large_img.BlockImageLoader;
...
public class LargeImageView extends View implements OnImageLoadListener, ILargeImageView {
    ...
    private int preCenterX = 0;
    private int preCenterY = 0;
...
//            preCenterX = scaleHelper.getStartX();
//            preCenterY = scaleHelper.getStartY();
...
//            configureBounds();
        ...
        if (mFactory == null) {
            ...
            if (BlockImageLoader.DEBUG) {
                for (int i = 0; i < drawDatas.size(); i++) {
                    com.vitec.task.smartrule.view.large_img.BlockImageLoader.DrawData data = drawDatas.get(i);
    ...
    private boolean overScrollByCompat(int deltaX, int deltaY,
        ...
        int distanceX = newScrollX - oldScrollX;
        int distanceY = newScrollY - oldScrollY;
        if (touchCallBack!=null && (distanceX != 0 || distanceY != 0)) {
//            LogUtils.show("overScrollByCompat-----查看偏移值：" + distanceX + "," + distanceY);
            if (flag == 0) {
//                LogUtils.show("非缩放情况下被调用了平移："+distanceX+","+distanceY);
                touchCallBack.onScroll(distanceX,distanceY);
            } else if (flag == 1) {
                touchCallBack.onScroll(distanceX,distanceY,flag);
            }
        }
    ...
    private GestureDetector.SimpleOnGestureListener simpleOnGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            ...
            overScrollByCompat((int) distanceX, (int) distanceY, getScrollX(), getScrollY(), getScrollRangeX(), getScrollRangeY(), 0, 0, false,0);
            return true;
        }
...
//            preCenterX = (int) detector.getFocusX();
//            preCenterY = (int) detector.getFocusY();
...
//        preCenterY = getMeasuredHeight() >> 1;
//        preCenterX = getMeasuredWidth() >> 1;
    ...
    public void setScale(float scale, int centerX, int centerY) {
        ...
        overScrollByCompat(dx, dy, sX, sY, getScrollRangeX(), getScrollRangeY(), 0, 0, false,1);
        notifyInvalidate();
        if (touchCallBack != null) {
//            设置其他图标的缩放值

            if (preScale != scale) {
                touchCallBack.onScale(preScale,scale,centerX,centerY);
                int detaleX = centerX - preCenterX;
                int detaleY = centerY - preCenterY;
//                  LogUtils.show("setScale-----查看centerX----" + centerX + "，"+centerY+"----上一次的preCenterX：" +preCenterX+","+preCenterY+"----缩放中心偏移值："+ detaleX + "," + detaleY);
                if (detaleX != 0 || detaleY != 0) {
//                    touchCallBack.onScroll(detaleX,detaleY,1);
//                    LogUtils.show("setScale-----查看缩放值----" + scale + "，上一次的缩放值：" +preScale+"----缩放中心偏移值："+ detaleX + "," + detaleY);
                }
